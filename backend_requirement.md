...
이 문서는 현재 프론트엔드(`tmap_web_fe`)가 **localStorage mock**으로 구현한 동작을 기준으로, 실제 백엔드에 필요한 기능·정책·데이터를 정리한다. TMAP 검색·지도 타일 호출은 클라이언트가 직접 TMAP API를 쓰는 전제이며, POI 메타데이터는 즐겨찾기 모음에 **스냅샷(JSON)**으로 저장된다.
...
- **DB**: PostgreSQL 등 관계형 DB 권장 (즐겨찾기 모음·피드백·사용자 연동).
...
- **즐겨찾기 모음**: 여러 목적지(POI) 묶음을 저장·공유·협업(좋아요·노트)한다.
- **사용자**: Google 계정으로 로그인한다. (프론트는 GIS + 백엔드 토큰 교환을 전제로 한다.)
- **개인화**: 집/직장 바로가기, 최근 목적지 등은 **로그인 사용자별**로 서버에 둔다. (즐겨찾기 서버 API는 제거 — 클라이언트 전용.)
- **공유**: 짧은 URL(`#/c/<clusterId>`)로 비로그인 사용자도 **열람**할 수 있다. **새 즐겨찾기 모음 생성**은 로그인 후에만 허용한다.
...
- 즐겨찾기 모음 **조회**(읽기)는 인증 없이 가능해야 한다 (현재 스펙: 공유받은 사용자는 비로그인으로 목록/상세 열람).
- **인증 필수(일반)**: 즐겨찾기 모음 생성, 목적지 추가/삭제, 즐겨찾기 모음 이름·지도 메타 수정, 소유자 삭제, 집/직장, 포크 등.
- **인증 필수 + 공유 수신자 허용**: 즐겨찾기 모음 **소유자가 아닌** 사용자(공유 링크로 접근한 **공유받은 사람**)도, **로그인한 상태**에서는 해당 즐겨찾기 모음의 목적지에 대해 **노트 작성·수정(본인 노트)·삭제(본인 노트)** 및 **좋아요(토글)** 가 가능해야 한다. 동일 피드백 데이터를 모든 열람자가 공유한다.
...
| 동작 | 인증 | 설명 |
|------|------|------|
| 생성 | 필수 | 최소 1개 POI로 즐겨찾기 모음 생성. `ownerId` 설정. |
| 단건 조회 | 선택 | 공유 링크로 비로그인 조회 허용. 존재하지 않으면 404. |
| 목록 “내 즐겨찾기 모음” | 선택 | **소유한 즐겨찾기 모음** + **구독·공유로 받은 즐겨찾기 모음** (아래 5.4). |
...
- 소유하지 않은 즐겨찾기 모음은 **읽기 전용** (`viewOnly`): **목적지(POI) 편집만** 불가. **좋아요·노트는 로그인한 공유받은 사람도 가능** (아래 6절).
- 리스트에서 **“내 목록에서 제거”**: 수신자 입장에서만 목록에서 숨김 (원본 즐겨찾기 모음은 유지).

**백엔드 요구**

- **공유 링크**: `clusterId`만으로 GET 가능 (비밀 추측 방지를 위해 ID 엔트로피 충분).
- **구독/수신 테이블** (권장): `userId` + `clusterId` + `hidden` 등으로 “내 목록에서 제거” 구현.
- 또는 **즐겨찾기 모음 사본을 로컬에만 두는** 현재 mock과 다르게, 서버에서 “수신함”과 “소유”를 분리하는 API 설계.

### 5.5 “내 계정에 복사” (포크)

- **인증 필수**.
- 원본 즐겨찾기 모음의 `pois`, `mapCenter`, `mapZoom`을 복사하되 **`feedback`은 비움** (프론트 `App.tsx` `onCopyAsMine`와 동일 정책: 타인 노트/좋아요를 사본에 옮기지 않음).
- 새 `id` 발급, 새 `ownerId`, 이름은 클라이언트가 제안 가능 (예: `"이름 (사본)"`).

---

## 6. 즐겨찾기 모음 피드백 (좋아요·노트)

공유된 즐겨찾기 모음에 대해 **모든 열람자가 동일 데이터**를 본다 (즐겨찾기 모음 문서에 embedded 또는 동일 `clusterId`로 조인).
...
- 목적지 변경·즐겨찾기 모음 삭제(소유) 등은 **소유자만** `403`으로 제한한다.
...
- **삭제**: 해당 노트 작성자 **또는** 즐겨찾기 모음 **소유자** (프론트: 소유자는 타인 노트 삭제 가능).
...
> **백엔드**: `GET/POST/DELETE /me/favorites` API 및 DB 테이블은 **제거됨**. 즐겨찾기는 **프론트 localStorage 등 클라이언트 전용**으로 유지한다.
...
| 항목 | 요구 |
|------|------|
| 즐겨찾기 모음 ID | 열거 공격 방지 — 길고 무작위. |
| 공개 조회 | 민감한 개인정보는 즐겨찾기 모음 본문에 넣지 않도록 가이드 (클라이언트 책임 + 서버 검증 선택). |
...
- **403**: 예) 비소유자가 **목적지·즐겨찾기 모음 메타** 변경 시도. **공유받은 사용자의 정상적인 노트/좋아요 요청은 403이 되면 안 된다.**
- **409**: ID 충돌, 중복 멤버십 등.
...
- 로깅(감사): 즐겨찾기 모음 삭제, 소유권 이전, 관리자 조회.
- 백업·복구: `clusters` 및 `feedback` 포함.
- 향후: 알림, 실시간 동기화(WebSocket), 즐겨찾기 모음 초대 링크 만료 등은 별도 이슈.
